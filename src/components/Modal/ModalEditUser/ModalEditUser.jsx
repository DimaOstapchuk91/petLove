...
const ModalEditUser = ({ onClose }) => {
  ...
  return (
    <div className='w-full max-w-[335px] py-10 px-5  rounded-[30px] bg-text-white'>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className='space-y-4'>
        {preview || avatar ? (
          <img
            src={preview || avatar}
            alt='Avatar'
            className='mx-auto rounded-full max-w-20 mb-3'
          />
        ) : (
        ...
        <div className='flex justify-between gap-2 mb-2.5'>
          <label className='block max-w-[161px]  text-sm font-medium leading-4.5 -tracking-[0.42]'>
            <input
              type='text'
          ...
          <button
            htmlFor='file-upload'
            className='flex items-center gap-2 cursor-pointer p-3 rounded-[30px] bg-brand-light !text-xs leading-4 -tracking-[0.24px] font-medium hover:bg-hover-light'
        ...
        <button
          type='submit'
          className='w-full p-3 bg-brand hover:bg-hover rounded-[30px] !text-sm font-bold text-text-white leading-4.5 -tracking-[0.42px] cursor-pointer'
        >
          Go to profile
        </button>
      </form>
